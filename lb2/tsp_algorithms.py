...
def reduce_cost_matrix(matrix):
    """Редукция матрицы: вычитает минимальные элементы строк и столбцов, уменьшая стоимость пути."""
    reduced_matrix = matrix.copy()
    row_min = np.min(reduced_matrix, axis=1)
    row_min[row_min == INF] = 0  # Учитываем бесконечность как отсутствие пути
    reduced_matrix -= row_min[:, None]
    col_min = np.min(reduced_matrix, axis=0)
    col_min[col_min == INF] = 0
    reduced_matrix -= col_min
    return reduced_matrix, np.sum(row_min) + np.sum(col_min)

def minimum_spanning_tree(matrix, vertices):
    """Вычисляет минимальное остовное дерево (MST) алгоритмом Прима для оценки нижней границы стоимости пути."""
    if len(vertices) <= 1:
        return 0.0
    total_cost, visited = 0.0, set()
    priority_queue = [(0.0, next(iter(vertices)))]
...
def tsp_branch_and_bound(matrix, current, visited, current_cost, path, best):
    ...
    if len(visited) == num_cities:
        ...
        if total_cost < best['cost']:
            ...
            best['path'] = path + [0]  # Замыкаем цикл
        return

    # Формируем список кандидатов с их стоимостями
    candidates = [(city, matrix[current][city]) for city in range(num_cities) if city not in visited and matrix[current][city] != INF]
    ...
    for next_city, cost_to_next in candidates:
        ...
        new_matrix[current, :] = INF  # Исключаем текущий город
        new_matrix[:, next_city] = INF  # Исключаем обратные пути

        if len(visited) + 1 < num_cities:
            new_matrix[next_city][0] = INF  # Исключаем преждевременное возвращение

        reduced_matrix, reduced_cost = reduce_cost_matrix(new_matrix)
        new_cost = current_cost + cost_to_next + reduced_cost

        # Альтернативная нижняя граница: сумма двух наименьших рёбер для каждой оставшейся вершины
        ...
        if lower_bound < best['cost']:
            tsp_branch_and_bound(reduced_matrix, next_city, visited | {next_city}, new_cost, path + [next_city], best)


def tsp_little_algorithm(matrix):
    """Решает задачу коммивояжера методом Литтла (ветви и границы)."""
